...
@section('content')
    <div class="container-fluid">
        @php
            ...
            $hasFilters = ($filters['search'] ?? '') !== '' || ($filters['trainer_id'] ?? null) || ($activeStatus !== 'all') || ($filters['start_date'] ?? null) || ($filters['end_date'] ?? null) || $showArchived;
                        ...
                        <form action="{{ route('admin.history.classes') }}" method="GET" id="class-history-filter-form" class="mt-3">
                            ...
                            <div class="d-flex flex-wrap gap-2 align-items-center mb-3">
                                @foreach ($statusLabels as $key => $label)
                                    ...
                                    <button
                                        type="button"
                                        class="btn btn-sm rounded-pill px-3 class-history-status-chip {{ $activeStatus === $key ? 'btn-danger' : 'btn-outline-secondary' }}"
                                        data-status="{{ $key }}"
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function renderPrintWindow(payload) {
                ...
                const html = `
                    ...
                    <html>
                        <head>
                            <title>${payload.title || 'Class history'}</title>
                            <style>
                        ...
                        <body>
                            <div class="sheet">
                                <div class="header">
                                    <div>
                                        <h1 class="title">${payload.title || 'Class history'}</h1>
                                ...
                                <table>
                                    ...
                                    <tbody>
                                        ${rows || '<tr><td colspan="7" style="text-align:center; padding:16px;">No classes available for this view.</td></tr>'}
                                    </tbody>
                                </table>
                            </div>
