...
@section('content')
    @php
        ...
        $seriesStartVal = old('series_start_date', $data->series_start_date ?? ($startDate ? $startDate->toDateString() : ''));
        $seriesEndVal = old('series_end_date', $data->series_end_date ?? ($endDate ? $endDate->toDateString() : ''));
        $startTimeVal = old('class_start_time', $startDate ? $startDate->format('H:i') : '');
        $endTimeVal = old('class_end_time', $endDate ? $endDate->format('H:i') : '');
        $recurringVal = old('recurring_days', $data->recurring_days ?? null);
        $hiddenStartVal = $startDate ? $startDate->format('Y-m-d\TH:i') : '';
        $hiddenEndVal = $endDate ? $endDate->format('Y-m-d\TH:i') : '';
    @endphp
    <style>
        .schedule-card {
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-12 d-flex justify-content-between">
                <div><h2 class="title">Edit Schedule</h1></div>
            </div>
            <div class="col-lg-12">
                <div class="box">
                    <div class="row">
                                        ...
                                        <div class="schedule-card p-4 rounded-4 border shadow-sm">
                                            ...
                                            <div class="row g-3">
                                                <div class="col-lg-6 col-sm-12">
                                                    ...
                                                    <input type="date" class="form-control" id="series_start_date" value="{{ $seriesStartVal }}">
                                                ...
                                                <div class="col-lg-6 col-sm-12">
                                                    ...
                                                    <input type="date" class="form-control" id="series_end_date" value="{{ $seriesEndVal }}">
                                                ...
                                                <div class="col-lg-6 col-sm-12">
                                                    ...
                                                    <input type="time" class="form-control" id="class_start_time" name="class_start_time" value="{{ $startTimeVal }}">
                                                </div>
                                                <div class="col-lg-6 col-sm-12">
                                                    <label for="class_end_time" class="form-label">Class End Time <span class="required">*</span></label>
                                                    <input type="time" class="form-control" id="class_end_time" name="class_end_time" value="{{ $endTimeVal }}">
                                                </div>
                                            </div>
                                            <div class="mt-3">
                                            ...
                                            <input type="hidden" id="class_start_date" name="class_start_date" value="{{ $hiddenStartVal }}" />
                                            <input type="hidden" id="class_end_date" name="class_end_date" value="{{ $hiddenEndVal }}" />
                                            <input type="hidden" id="recurring_days" name="recurring_days" value="{{ is_array($recurringVal) ? json_encode($recurringVal) : $recurringVal }}" />
                                            <input type="hidden" id="series_start_date_hidden" name="series_start_date" value="{{ $seriesStartVal }}" />
                                            <input type="hidden" id="series_end_date_hidden" name="series_end_date" value="{{ $seriesEndVal }}" />
                                        </div>
                                    </div>
                                </div>
                                @php
                                        ...
                                        <select class="form-control" id="trainer_id" name="trainer_id" required>
                                            @foreach($trainers as $trainer)
                                                ...
                                                <option
                                                    ...
                                                    {{ (string) $selectedTrainerId === (string) $trainer->id ? 'selected' : '' }}
                                                >
                                                    {{ $trainerName }}
                                                </option>
                                            @endforeach
    ...
    <script>
        ...
        const serverSeriesStart = @json($seriesStartVal);
        const serverSeriesEnd = @json($seriesEndVal);
        const serverStartTime = @json($startTimeVal);
        const serverEndTime = @json($endTimeVal);
        const existingRecurringRaw = @json($recurringVal);
        ...
        const parseExistingRecurring = () => {
            if (!existingRecurringRaw) return null;
            try {
                const parsed = typeof existingRecurringRaw === 'string' ? JSON.parse(existingRecurringRaw) : existingRecurringRaw;
        ...
        const initializeDefaults = () => {
            if (serverSeriesStart && seriesStartInput && !seriesStartInput.value) {
                seriesStartInput.value = serverSeriesStart;
            }
            if (serverSeriesEnd && seriesEndInput && !seriesEndInput.value) {
                seriesEndInput.value = serverSeriesEnd;
            }
            if (serverStartTime && startTimeInput && !startTimeInput.value) {
                startTimeInput.value = serverStartTime;
            }
            if (serverEndTime && endTimeInput && !endTimeInput.value) {
                endTimeInput.value = serverEndTime;
            }

            const parsedRecurring = parseExistingRecurring();
            if (parsedRecurring && parsedRecurring.length) {
                selectedDays = new Set(parsedRecurring);
            } else if (seriesStartInput?.value) {
                const startDay = new Date(seriesStartInput.value + 'T00:00:00').getDay();
                const startKey = daysMeta[startDay]?.key;
                if (startKey) selectedDays.add(startKey);
            } else {
            ...
            renderSelectedDays();
            updateHiddenFields();
            calendarCursor = seriesStartInput?.value ? new Date(seriesStartInput.value + 'T00:00:00') : new Date();
            renderCalendar();
        };
        initializeDefaults();
